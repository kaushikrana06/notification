...
class BrowseAdapter extends RecyclerView.Adapter<BrowseViewHolder> {
    ...
    private final static String PAGE_SIZE = "10999";
    ...
    private ArrayList<BrowseAdapter.DataItem> data;
    ...
    public void onBindViewHolder(@NonNull BrowseViewHolder vh, int position) {
        BrowseAdapter.DataItem item = data.get(position);
    ...
    public void filterList(ArrayList<BrowseAdapter.DataItem> filteredList) {
        data = filteredList;
    ...
    public ArrayList filter(String text) {
        ArrayList<BrowseAdapter.DataItem> filteredList = new ArrayList<>();

        {
            for (BrowseAdapter.DataItem item : data) {
                if (item.appName != null && !item.appName.equals("") && (item.getAppName().toLowerCase().contains(text.toLowerCase()) ||
